...
// Pin definitions (adjust if needed based on your hardware wiring)
sbit NSS = P1^5;    // SPI Slave Select (Chip Select for LoRa)
sbit DIO1 = P1^1;   // DIO1 for interrupts
sbit RESET = P1^3;  // Reset pin

// Global configuration variables (ported from class members)
unsigned long pllFrequency;
unsigned long freq_hz = 915000000UL; // Default 915 MHz
...
// Delay in milliseconds (using provided Timer0_Delay)
void Delay_ms(unsigned int ms) {
    Timer0_Delay(24000000, 1000, ms);
}

// Convert frequency to PLL (ported directly)
unsigned long frequencyToPLL(long rfFreq) {
...
// Wait for radio command completion (poll status with timeout approximation)
unsigned char Lora_waitForRadioCommandCompletion(unsigned long timeout_ms) {
    int loops;
...
// Configure essential radio settings (minimal setup)
void Lora_configureRadioEssentials(void) {
    ...
    Lora_configSetFrequency(915000000);
    ...
    Lora_configSetPreset(PRESET_DEFAULT);
    ...
    NSS = 0;
    Spi_Write_Byte(0xA0);
    Spi_Write_Byte(0x00); // 0 symbols (ping-pong example uses 5, but default 0)
    NSS = 1;
    Delay_ms(100);
...
int Lora_receive_async(unsigned char *buff, int buffMaxLen) {
    ...
    // Get packet status (optional, for RSSI/SNR)
    NSS = 0;
    Spi_Write_Byte(0x14);
...
// Set frequency
unsigned char Lora_configSetFrequency(long frequencyInHz) {
    if (frequencyInHz < 150000000 || frequencyInHz > 960000000) return 0;
...
// Set bandwidth (validate against datasheet values)
unsigned char Lora_configSetBandwidth(unsigned char bw) {
...
void main(void) {
...
#ifdef MODE_TX
    while (1) {
        sprintf((char*)buffer, "%u", counter);
        printf("Sending: %s\n", buffer);
        Lora_transmit(buffer, strlen((char*)buffer));
        printf("Sent!\n");
        counter++;
    ...
    while (1) {
        printf("Listening...\n");
        len = (unsigned char)Lora_receive_blocking(buffer, 64, 0); // Infinite timeout
        if (len > 0) {
            buffer[len] = '\0';
